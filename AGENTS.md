...
- `src/scheduler/` — deterministic constraint-satisfaction solver (pure, side-effect-free, unit-tested). Preserve determinism: no randomness, sort/iterate inputs canonically. `solve()` = `normalizeInput` → `cspSolve` (branch-and-bound CP in `search.ts`, greedy `solveCore` as completeness fallback when `unassigned > 0`) → `assignRooms` (keep instructor's room unless a schedule break) → `evaluateConstraints`. Both solver paths are deterministic and share the `SolveResult` interface, so the search internals can be swapped without moving the app.
...
- `src/lib/api.ts` now has `createX`/`updateX`/`deleteX` for semesters, persons, courses, classrooms, plus preferences/locks/constraints helpers, `setQualification`, and schedule helpers (`createScheduleRun`, `fetchScheduleAssignments`, `deleteScheduleRun`). Keep the mutate-helpers-return-id rule there.
- **Export** (`src/lib/export.ts` pure + `src/lib/export-ui.tsx` lazy wrapper): CSV-by-course, CSV-by-teacher, and the PCO xlsx layout, wired into the Schedule page for any shown run. The SheetJS (`xlsx`) runtime is code-split (loaded on first export click). Export tests in `src/lib/__tests__/export.test.ts`.

## Key domain rules
- 12 periods, 12 slots, named **M1–M6 / T1–T6**. **LUNCH BREAK between M4 and M5** (and T4/T5): those are NOT consecutive — a teacher on M4+M5 has a break. `src/lib/periods.ts` encodes adjacency for this. The valid **double-period blocks** are exactly (1,2), (3,4), (5,6) (a double course starts only at the 1st/3rd/5th slot).
- **Course load is a per-person TARGET** (default 3; `new_instructor` default 4), reduced manually for extra responsibilities (pilot, double-prep, etc.). It is a soft goal, not a hard cap. Manual input always wins. The solver treats it as a soft `load_target` constraint (penalty 20, seeded with the defaults) so an instructor is never assigned more sections than their target.
...
- Everything else is a **soft constraint**, each with an integer penalty (higher = relaxed/violated later). Known types: spread_sections, morning_min, afternoon_min, balance_mt, consecutive_periods, single_day (all-M or all-T), no_forced_break, single_offering_peak, two_section_same_block, **load_target**.
- **Double-period course** (`course_list.is_double_period`): the solver places each section as a 2-slot block starting only at the 1st/3rd/5th slot and reserves both periods for one person; `assignRooms` holds ONE real room across both periods. NONE/independent-study rooms are a manual AD lock and the solver never places them.
